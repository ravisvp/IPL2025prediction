    ...
    for p in preds:
        ...
        for i in range(total_games_entered):
           if i < len(user_game_preds):
               user_pick = user_game_preds[i].strip().upper()
               actual = actual_results[i].strip().upper()
        ...
        p.points = points
        p.hits = hits_league
        # Recalculate dynamic misses only for non-blank results
        misses = sum(1 for i in range(total_games_entered)
             if i < len(user_game_preds)
             and actual_results[i].strip().upper() != ""
             and actual_results[i].strip().upper() != "WASHOUT"
             and user_game_preds[i].strip().upper() != actual_results[i].strip().upper())
        p.misses = misses
